...
docker exec -i postgres_db psql -U postgres -d listflowhq -c "SELECT COUNT(*) FROM \"Catalog\";" > temp_catalog_count.txt
for /f "tokens=2 delims=|" %%a in ('findstr /c:"count" temp_catalog_count.txt') do set CATALOG_COUNT=%%a
set CATALOG_COUNT=%CATALOG_COUNT: =%
if "%CATALOG_COUNT%"=="0" (
    echo Error: Catalog table is empty after seeding!>> f:\Dev\ListFlowHQ\backend\scripts\build.log
    echo Error: Catalog table is empty after seeding!
    exit /b 1
) else (
    echo Catalog table contains %CATALOG_COUNT% records after seeding.>> f:\Dev\ListFlowHQ\backend\scripts\build.log
