...
echo. > f:\Dev\ListFlowHQ\backend\scripts\build.log
...
docker volume inspect listflowhq-db >nul 2>&1
if %ERRORLEVEL% neq 0 (
    echo External volume listflowhq-db not found. Creating it now...
    docker volume create listflowhq-db >nul 2>&1
    if %ERRORLEVEL% neq 0 (
    echo Error: Failed to create required external volume listflowhq-db.
        exit /b 1
    )
    echo External volume listflowhq-db created.
)
...
docker-compose -f f:\Dev\ListFlowHQ\docker-compose.yml up --build -d
if %ERRORLEVEL% neq 0 (
...
:run_migrations
REM Ensure the database exists before applying schema
docker exec -i listflowhq-db psql -U postgres -c "CREATE DATABASE listflowhq;" 2>nul
REM Single canonical schema apply (01_init executed only on brand-new container by docker-entrypoint-initdb.d)
docker exec -i listflowhq-db psql -U postgres -d listflowhq < f:/Dev/ListFlowHQ/backend/database/recreate_schema.sql
if %ERRORLEVEL% neq 0 (
...
node f:\Dev\ListFlowHQ\backend\scripts\build.js
if %ERRORLEVEL% neq 0 (
...
echo Build process completed successfully. >> f:\Dev\ListFlowHQ\backend\scripts\build.log
exit /b 0
