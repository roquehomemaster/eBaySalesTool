...
function setEnvironmentVariablesFromConfig(config) {
    ...
    process.env.POSTGRES_DB_IP = config.network.postgres_db_ip;
    process.env.BACKEND_IP = config.network.backend_ip;
    process.env.FRONTEND_IP = config.network.frontend_ip;
}

const { maxRetries, retryDelay, healthCheckRetries, healthCheckDelay } = config.constants;

function runCommand(command, args, options = {}) {
    // On Windows, explicitly use npm.cmd for npm commands
...
function seedDatabase(config) {
    if (config.testdata) {
        log('Seeding database with test data...');
        try {
            runCommand('node', [path.join(config.paths.buildScripts, 'seedDatabase.js')], { stdio: 'inherit' });
...
async function ensureContainersStopped() {
    ...
    try {
        ...
        if (runningContainers.length > 0 && runningContainers[0] !== '') {
...
function waitForAllContainers(config) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const expectedContainers = ['postgres_db', 'ebaysalestool-backend-1', 'ebaysalestool-frontend-1'];
